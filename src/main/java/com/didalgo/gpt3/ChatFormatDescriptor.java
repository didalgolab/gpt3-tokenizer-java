...
public interface ChatFormatDescriptor {
    ...
    static ChatFormatDescriptor forModel(String modelName) {
        return switch (modelName) {
            ...
            case "gpt-3.5-turbo-0301" -> new Of(Encoding.forModel(modelName), 4, 3, Of.UNSUPPORTED);
            case "gpt-4-0314", "gpt-4-32k-0314" -> new Of(Encoding.forModel(modelName), 3, 3, Of.UNSUPPORTED);
            case "gpt-3.5-turbo-0613", "gpt-3.5-turbo-16k-0613",
                "gpt-4-0613", "gpt-4-32k-0613" -> new Of(Encoding.forModel(modelName), 3, 3, -1);
            default -> throw new IllegalArgumentException(String.format("Model `%s` not found", modelName));
        };
    }

    record Of (Encoding encoding, int extraTokenCountPerMessage, int extraTokenCountPerRequest, int extraTokenCountForFunctions) implements ChatFormatDescriptor {
